...
import { createMainMenu, createBackButton, createStopButton, createAccountSwitchKeyboard, createGroupsMenu, createConfigMenu, createQuietHoursKeyboard, createScheduleKeyboard, createMessagePoolKeyboard, createMessagePoolListKeyboard, generateStatusText, createLoginOptionsKeyboard, createMessagesMenu, createSavedTemplatesKeyboard } from './keyboardHandler.js';
 ...
 * @param {Object} e - GramJS entity object
 * @returns {string} Bot API entity type
 */
...
export async function handlePoolAddMessage(bot, callbackQuery) {
  ...
  try {
    ...
    // Get all messages from Saved Messages (no limit)
    ...
    if (!messages || messages.length === 0) {
      const instructions = `📝 <b>Add Messages to Pool</b>\n\n` +
        ...
        `The bot will automatically add <b>all messages</b> from your Saved Messages.`;
    ...
    // Process all messages from Saved Messages
    let addedCount = 0;
...
export async function handlePoolClear(bot, callbackQuery) {
  ...
  // Delete all messages (including inactive ones)
  for (const msg of pool) {
    await messageService.deleteFromMessagePool(accountId, msg.id);
  }

  // Disable pool since it's now empty
  await configService.setMessagePoolEnabled(accountId, false);
