...
class AccountLinker {
   ...
   */
  async disconnectAll() {
    console.log('[ACCOUNT_LINKER] Disconnecting all clients...');
    const disconnectPromises = [];

    // Disconnect linked accounts
    for (const [accountId, account] of this.linkedAccounts.entries()) {
      if (account.client) {
        try {
          if (account.client.connected) {
            disconnectPromises.push(account.client.disconnect().catch(e => 
              console.log(`[SHUTDOWN] Error disconnecting account ${accountId}:`, e.message)
            ));
          }
        } catch (e) {
          // Ignore errors checking connected status
        }
      }
    }

    // Disconnect pending verifications
    for (const [userId, pending] of this.pendingVerifications.entries()) {
      if (pending.client) {
        disconnectPromises.push(pending.client.disconnect().catch(() => {}));
      }
    }

    // Disconnect pending password auth
    for (const [userId, pending] of this.pendingPasswordAuth.entries()) {
      if (pending.client) {
        disconnectPromises.push(pending.client.disconnect().catch(() => {}));
      }
    }

    // Disconnect pending web logins
    for (const [userId, pending] of this.pendingWebLogins.entries()) {
      if (pending.client) {
        disconnectPromises.push(pending.client.disconnect().catch(() => {}));
      }
    }
    ...
    console.log(`[ACCOUNT_LINKER] Disconnected ${disconnectPromises.length} clients`);
  }
