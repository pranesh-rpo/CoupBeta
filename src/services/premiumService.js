...
class PremiumService {
  ...
  /**
   * Manually set expiry date for a subscription
   * @param {number} userId - User ID
   * @param {Date|string} expiryDate - Expiry date (Date object or ISO string)
   * @returns {Promise<{success: boolean, error?: string, subscription?: Object}>}
   */
  async setExpiryDate(userId, expiryDate) {
    try {
      const expiresAt = expiryDate instanceof Date ? expiryDate : new Date(expiryDate);
      ...
      if (!existing) {
        // Create new subscription with custom expiry
        await db.query(
          `INSERT INTO premium_subscriptions 
           (user_id, status, amount, currency, payment_method, payment_reference, expires_at)
           VALUES ($1, 'active', $2, $3, $4, $5, $6)`,
          [userId, 30.0, 'INR', 'admin_manual', `Admin set date: ${expiresAt.toISOString()}`, expiresAt.toISOString()]
        );
        logger.logChange('PREMIUM', userId, `Premium subscription created with custom expiry: ${expiresAt.toISOString()}`);
      } else {
        ...
        logger.logChange('PREMIUM', userId, `Premium subscription expiry date set to: ${expiresAt.toISOString()}`);
      }

      const subscription = await this.getSubscription(userId);
      return { success: true, subscription };
    } catch (error) {
      logger.logError('PREMIUM', userId, error, 'Failed to set expiry date');
      return { success: false, error: error.message };
    }
  }

  /**
   * Extend subscription by number of days
   * @param {number} userId - User ID
  ...
  async extendSubscription(userId, days) {
    try {
      if (!days || days <= 0 || !Number.isInteger(days)) {
        return { success: false, error: 'Invalid number of days. Must be a positive integer.' };
      }
      ...
      if (!existing) {
        ...
        await db.query(
          `INSERT INTO premium_subscriptions 
           (user_id, status, amount, currency, payment_method, payment_reference, expires_at)
           VALUES ($1, 'active', $2, $3, $4, $5, $6)`,
          [userId, 30.0, 'INR', 'admin_manual', `Admin extended by ${days} days`, expiresAt.toISOString()]
        );
        logger.logChange('PREMIUM', userId, `Premium subscription created and extended by ${days} days`);
      } else {
        // Extend existing subscription
        const currentExpiresAt = new Date(existing.expires_at);
        const now = new Date();
        ...
        logger.logChange('PREMIUM', userId, `Premium subscription extended by ${days} days until ${newExpiresAt.toISOString()}`);
      }

      const subscription = await this.getSubscription(userId);
      return { success: true, subscription };
    } catch (error) {
      logger.logError('PREMIUM', userId, error, 'Failed to extend subscription');
      return { success: false, error: error.message };
    }
  }

  /**
   * Remove premium subscription (cancel it)
   * @param {number} userId - User ID
   * @returns {Promise<{success: boolean, error?: string}>}
   */
  async removePremium(userId) {
    try {
      const result = await this.cancelSubscription(userId);
      if (result.success) {
        logger.logChange('PREMIUM', userId, 'Premium subscription removed by admin');
      }
      return result;
    } catch (error) {
      logger.logError('PREMIUM', userId, error, 'Failed to remove premium');
      return { success: false, error: error.message };
    }
  }
   ...
   * @returns {Promise<{success: boolean, error?: string, subscription?: Object}>}
   */
  async updateAmount(userId, amount) {
    try {
      ...
      logger.logChange('PREMIUM', userId, `Premium subscription amount updated to ₹${amount}`);
      
      const subscription = await this.getSubscription(userId);
      return { success: true, subscription };
    } catch (error) {
      logger.logError('PREMIUM', userId, error, 'Failed to update amount');
      return { success: false, error: error.message };
    }
  }
}
