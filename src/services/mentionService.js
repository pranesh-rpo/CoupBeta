  ...
  async addMentionsToMessage(client, groupEntity, messageText, mentionCount = 5, excludeUserId = null) {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Mention process timed out after 5s')), 5000)
      );

      return await Promise.race([mentionPromise, timeoutPromise]);
    } catch (error) {
      logError('MENTION_SERVICE', null, error, `Error adding mentions: ${error.message}`);
