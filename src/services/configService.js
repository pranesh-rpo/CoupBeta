...
class ConfigService {
    ...
    try {
      const result = await db.query(
        `SELECT 
          ...
          auto_reply_native_mode,
          auto_reply_shortcut_id,
          use_message_pool,
          message_pool_mode,
      ...
      return {
        ...
        autoReplyCheckInterval: row.auto_reply_check_interval !== null && row.auto_reply_check_interval !== undefined ? row.auto_reply_check_interval : 30,
        autoReplyNativeMode: row.auto_reply_native_mode === 1,
        autoReplyShortcutId: row.auto_reply_shortcut_id || null,
  ...
  /**
   * Set auto reply native mode and shortcut ID
   */
  async setAutoReplyNativeMode(accountId, enabled, shortcutId = null) {
    try {
      await db.query(
        `UPDATE accounts
         SET auto_reply_native_mode = $1, auto_reply_shortcut_id = $2, updated_at = CURRENT_TIMESTAMP
         WHERE account_id = $3`,
        [enabled ? 1 : 0, shortcutId, accountId]
      );
      logger.logChange('CONFIG', accountId, `Auto reply native mode ${enabled ? 'enabled' : 'disabled'}`);
      return { success: true };
    } catch (error) {
      logger.logError('CONFIG', accountId, error, 'Failed to set auto reply native mode');
      return { success: false, error: error.message };
    }
  }
