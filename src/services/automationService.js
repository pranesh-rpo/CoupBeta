...
/**
 * Resolve GramJS entity className to Bot API entity type string
 */
function resolveEntityType(e) {
      ...
      for (let i = 0; i < groupsToSend.length; i++) {
        ...
        try {
          ...
          if (useForwardMode) {
            ...
            let savedMessageIdToUse = lastSavedMessageId;
            ...
            if (!isPoolActive && lastSavedMessage) {
              ...
              if (lastSavedMessage.entities && lastSavedMessage.entities.length > 0) {
                entitiesToUse = lastSavedMessage.entities.map(e => {
                  const entityType = resolveEntityType(e);
                  ...
                  if (entityType === 'text_link' && e.url !== undefined) {
                    entity.url = e.url;
                  }

                  if (entityType === 'text_mention' && e.userId !== undefined) {
                    entity.user = { id: String(e.userId) };
                  }
              ...
              console.log(`[BROADCAST] Group "${groupName}": Using message pool text and entities (skipping Saved Messages override)`);
            } else {
              ...
              for (const entity of entitiesToUse) {
                try {
                  if (entity.type === 'custom_emoji' && entity.custom_emoji_id) {
                    ...
                    entities.push(new Api.MessageEntityUnderline({
                      offset: entity.offset,
                      length: entity.length
                    }));
                  } else if (entity.type === 'strikethrough') {
                    entities.push(new Api.MessageEntityStrike({
                      offset: entity.offset,
                      length: entity.length
                    }));
                  } else if (entity.type === 'spoiler') {
                    entities.push(new Api.MessageEntitySpoiler({
                      offset: entity.offset,
                      length: entity.length
                    }));
              ...
              try {
                ...
                if (hasPremiumEmojiEntities || (hasMentionEntities && hasOtherEntities)) {
                  ...
                  if (!messageValidation.valid) {
                    console.log(`[BROADCAST] ⚠️ ${messageValidation.error}`);
                    loggingService.logWarning(accountId, messageValidation.error, userId);
                    errorCount++;
                    continue;
                  }

                  // IMPORTANT: Use original messageToSend (not sanitized) when sending with entities
                  // Sanitization can shift character offsets, breaking entity positions (bold, italic, emoji, etc.)
                ...
                if (isPremiumEmojiError || (hasPremiumEmojiEntities && sendError?.message?.includes('emoji'))) {
                  ...
                  // Don't throw - allow fallback to send message without entities
                  // This ensures the message still gets sent, just without the premium emojis
                  console.log(`[BROADCAST] ⚠️ Proceeding to fallback: Sending message without premium emoji entities`);
          ...
          if (errorMsg.includes('readUInt32LE') || 
            ...
            try {
              ...
              if (useForwardMode && forwardMessageId && savedMessagesEntity) {
                ...
                if (autoMention) {
                  try {
                    // Use addMentionsToMessage for hidden mentions (modern replacement for getRandomMentions)
                    const mentionResult = await mentionService.addMentionsToMessage(client, group.entity, message, mentionCount, cachedExcludeUserId);
                    
                    if (mentionResult && mentionResult.message) {
                      messageToSend = mentionResult.message;
                      
                      // Convert internal entities to GramJS entities
                      if (mentionResult.entities && mentionResult.entities.length > 0) {
                        entities = mentionResult.entities.map(e => new Api.MessageEntityTextUrl({
                          offset: e.offset,
                          length: e.length,
                          url: e.url
                        }));
                        console.log(`[BROADCAST] Added ${entities.length} hidden mentions to retry message`);
                      } else {
                        entities = [];
                      }
                    }
                  } catch (mentionError) {
                    console.log(`[BROADCAST] ⚠️ Failed to add mentions during retry: ${mentionError.message}`);
                ...
                // Use original messageToSend when entities are present to preserve offsets
                const retryMessageToUse = (entities && entities.length > 0) ? messageToSend : (retryValidation.sanitized || messageToSend);

                if (retryMessageToUse && retryMessageToUse.trim().length > 0) {
                  await client.sendMessage(group.entity, { message: retryMessageToUse, entities });
