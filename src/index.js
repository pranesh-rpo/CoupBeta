...
import { validateUserId, validateAccountId, validateCallbackData, sanitizeErrorMessage, getUserFriendlyErrorMessage, adminCommandRateLimiter, verifyAccountOwnership, sanitizeString } from './utils/security.js';
import fs from 'fs';
...
import autoReplyIntervalService from './services/autoReplyIntervalService.js';
import autoReplyPollingService from './services/autoReplyPollingService.js';
import autoReplyRealtimeService from './services/autoReplyRealtimeService.js';
import scheduledMessageProcessor from './services/scheduledMessageProcessor.js';
...
function checkProcessLock() {
  try {
    if (fs.existsSync(LOCK_FILE)) {
      ...
      // If the lock file has the same PID as the current process, it's a stale lock file
      // from a previous run (likely inside a container where PIDs are reused).
      if (lockPid === process.pid) {
        console.log('⚠️  Found stale lock file with same PID ' + lockPid + ' (likely container restart). Overwriting...');
        // Do not exit, just proceed to overwrite the lock file
      } else {
        try {
          // Check if process is still running (Unix/Mac)
          process.kill(lockPid, 0); // Signal 0 doesn't kill, just checks if process exists
          
          // CRITICAL: In container environments (like Coolify/Docker), PID 1 is the init process.
          // If the lock file says PID 1 (from a previous run where bot was entrypoint),
          // but now the bot is a child process (PID > 1), we shouldn't exit just because PID 1 exists.
          if (lockPid === 1 && process.pid !== 1) {
            console.log('⚠️  Found lock file with PID 1. In containers, PID 1 is the init process.');
            console.log('   Assuming this is a stale lock file from a previous run. Overwriting...');
...
(async () => {
  try {
    ...
    // autoReplyRealtimeService is imported at the top
...
bot.on('message', async (msg) => {
  ...
  // CRITICAL: Only respond to private chats (after handling admin broadcast channel)
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  // CRITICAL: Only respond to private chats
  if (!isPrivateChat(callbackQuery.message)) {
    try {
      await bot.answerCallbackQuery(callbackQuery.id, {
  ...
  try {
    if (data.startsWith('otp_')) {
      ...
      await handleTemplateSelect(bot, callbackQuery, slot === 'none' ? null : parseInt(slot));
    } else if (data.startsWith('template_clear_')) {
      const slot = parseInt(data.replace('template_clear_', ''));
      if (!isNaN(slot)) {
...
async function gracefulShutdown(signal) {
  ...
  try {
    ...
    try {
      ...
      if (automationService && typeof automationService.stopAllBroadcasts === 'function') {
        automationService.stopAllBroadcasts();
      } else {
        // Fallback
        automationService.cleanupStoppedBroadcasts();
      }
      ...
      if (scheduledMessageProcessor && typeof scheduledMessageProcessor.stop === 'function') {
        scheduledMessageProcessor.stop();
      }
      
      // Stop auto-reply services
      if (autoReplyIntervalService && typeof autoReplyIntervalService.stopAll === 'function') {
        autoReplyIntervalService.stopAll();
      }
      
      if (autoReplyPollingService && typeof autoReplyPollingService.stopAll === 'function') {
        autoReplyPollingService.stopAll();
      }
      
      if (autoReplyRealtimeService && typeof autoReplyRealtimeService.stop === 'function') {
        autoReplyRealtimeService.stop();
      }
      
      // Stop logger bot service
      if (loggerBotService && typeof loggerBotService.stop === 'function') {
        loggerBotService.stop();
      }
      
      // Disconnect all Telegram clients
      if (accountLinker && typeof accountLinker.disconnectAll === 'function') {
        await accountLinker.disconnectAll();
      }
    } catch (error) {
